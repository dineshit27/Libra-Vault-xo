...
const features: Feature[] = [
  { 
    ...
    color: "bg-primary",
    image: "https://i.pinimg.com/1200x/c7/71/5d/c7715d857c258f4de4921cfe3f057e92.jpg"
  },
  { 
    icon: Search, 
    title: "Smart Search", 
    ...
    color: "bg-secondary",
    image: "https://i.pinimg.com/1200x/27/d5/c4/27d5c4bbbed8f5f3d8e1b7d854e5534f.jpg"
  },
  { 
    icon: Clock, 
    title: "Track Returns", 
    ...
    color: "bg-accent",
    image: "https://i.pinimg.com/736x/77/4f/eb/774feb5f288a559520b23122f22b20ff.jpg"
  },
  { 
    icon: Shield, 
    title: "Secure Access", 
    ...
    color: "bg-success",
    image: "https://i.pinimg.com/1200x/51/45/c1/5145c1f19ea5481e26d03394324b41b0.jpg"
  },
  { 
    icon: Star, 
    title: "Rate & Review", 
    ...
    color: "bg-primary",
    image: "https://i.pinimg.com/736x/2f/c3/0e/2fc30e6d39b75f37d9d189068176c607.jpg"
  },
  { 
    icon: Zap, 
    ...
    color: "bg-secondary",
    image: "https://i.pinimg.com/1200x/4a/52/86/4a528612fe8a5c2b159c33e968e7b21d.jpg"
  },
];
...
const FeaturesSection = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context((self) => {
      const panels = self.selector!(".feature-panel") as Element[];
      ...
      panels.forEach((panel) => {
        ...
        const icon = panel.querySelector(".card-icon");
  ...
  return (
    <section id="features" className="relative">
      <div className="pt-16 pb-4 px-6 text-center">
        ...
        <p className="text-muted-foreground font-body">Scroll to explore</p>
      </div>

      <div ref={wrapperRef} className="flex overflow-hidden relative">
        {features.map((f, i) => (
          <div
            key={f.title}
            className="feature-panel flex-shrink-0 w-screen h-screen flex items-center justify-center relative overflow-hidden"
            style={{
              backgroundImage: `url(${f.image})`,
              backgroundSize: "cover",
              backgroundPosition: "center",
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center text-center px-8 max-w-2xl">
              <div
                className={`card-icon ${f.color} w-20 h-20 rounded-2xl brutal-border flex items-center justify-center mb-8`}
              >
                <f.icon className="w-10 h-10 text-foreground" />
              </div>
              <h3 className="card-title font-heading text-5xl md:text-7xl font-black uppercase text-white mb-6 leading-none">
                {f.title}
              </h3>
              <p className="card-desc font-body text-white/80 text-lg leading-relaxed max-w-md">
                {f.desc}
              </p>
            </div>

            {/* Card counter */}
            <div className="absolute bottom-8 right-8 z-10 font-heading font-black text-white/40 text-6xl select-none">
...
const TrustedUsers = () => {
  const { count, ref } = useCountUp(99, 2000);
  return (
    <div className="flex flex-col items-center justify-center bg-transparent text-foreground py-4 px-4">
      <div className="flex -space-x-3 mb-2">
        {avatars.map((avatar, i) => (
          <div
            key={i}
            className={cn(
              "w-10 h-10 rounded-full overflow-hidden ring-2 ring-offset-2 ring-offset-background",
        ...
        <div className="w-10 h-10 rounded-full flex items-center justify-center bg-primary text-primary-foreground font-heading font-black text-[11px] ring-2 ring-offset-2 ring-offset-background ring-primary flex-shrink-0">
          +99
        </div>
      </div>
      <div className="flex flex-col items-center gap-1">
        ...
        <p ref={ref} className="text-sm font-body text-muted-foreground text-center">
          Join{" "}
          <span className="font-heading font-black text-foreground text-base">
            {count}+
          </span>{" "}
          happy members already using LibraVault
        </p>
      </div>
    </div>
  );
